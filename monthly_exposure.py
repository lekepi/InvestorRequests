...
def get_monthly_exposure(classification_list, numerator_list,
    ...
    if 'MarketCap' in classification_list:
        ...
        df_exposure['MarketCap'] = df_exposure.groupby('ticker')['marketCap'].fillna(method='ffill')
        df_exposure['MarketCap'] = df_exposure.apply(lambda x: 'Index' if x['ticker'] in ('ES1 CME', 'SXO1 EUX') else x['MarketCap'], axis=1)

    df_exposure['notional_usd'] = df_exposure['notional_usd'].fillna(0)
    df_exposure['Sector'] = df_exposure.apply(lambda x: 'Index' if x['ticker'] in ('ES1 CME', 'SXO1 EUX') else x['Sector'], axis=1)
    df_exposure['Country'] = df_exposure.apply(lambda x: 'S&P500' if x['ticker'] in ('ES1 CME') else x['Country'], axis=1)
    df_exposure['Country'] = df_exposure.apply(lambda x: 'Stoxx 600' if x['ticker'] in ('SXO1 EUX') else x['Country'], axis=1)

    df_exposure.sort_values(by='month_year', ascending=False, inplace=True)

    if is_leveraged and denominator == 'Nav':
        df_exposure['Exposure'] = df_exposure['Exposure'] * 2
    ...
    for classification in classification_list:
        ...
        for numerator in numerator_list:
            ...
            df_temp = df_temp.groupby(['month_year', classification]).agg({f'Exposure': 'sum'}).reset_index()
            df_pivot = df_temp.pivot(index='month_year', columns=classification, values='Exposure').fillna(0)
    ...
    file_name = f'Excel/Monthly {numerator_str}Exposure Over {denominator}.xlsx'
